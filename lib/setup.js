...
function gruntAction(content, next) {
  ...
  async.series([
        ...
        Object.keys(deps).forEach(function(depName) {
          tasks.push(function(next) {
            npm.load(function (err, npm) {
              ...
              npm.commands.install(process.cwd(), util.format('%s@"$s"', depName, deps[depName]), function(err, result) {
                if (err) {
    ...
    function(next) {
      ...
      try {
        console.log(util.format('loading %s.', gpath));
        g = require(gpath);
        console.log(util.format('loading %s.', gruntFilePath));
    ...
    function(next) {
      ...
      if (devInstalled && devInstalled.length >= 1) {
        ...
        common.out.info(util.format('now remove dev dependencies: %j.', devInstalled));
