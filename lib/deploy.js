  ...
  function _getEnsureLinkFunc(from, to) {
    return function(next) {
      ...
      try {
        ...
        if (stat) {
          common.out.warn(util.format('dst %s already exist, you may want to further inspect what it is.', to));
        }
      } catch() {}
      fs.symlink(from, to,  function(err) {
        // tolarent link error, just warning about that
        if (err) {
          common.out.warn(util.format('%j', err));
        }
        next();
      });
    };
  ...
  npm.load(function (err, npm) {
    ...
    npm.commands.install([pkg], function(err, result) {
      ...
      ensureSupportDirs(pkginfo[1], function(err) {
        ...
        var pkgInstalledPath = pkginfo[1];
        var pkgInstalledName = path.basename(pkginfo[1]);
