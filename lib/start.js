...
// to provide hooks while preserve most functions
function _fe_startDaemon(script, options, hooks) {
  ...
  //
  var _utile_randomString = function (length) {
  ...
  monitorPath = path.resolve(__dirname, '..', 'node_modules', 'forever', 'bin', 'monitor');
  ...
  hooks(monitor, function _fe_detach(monitor) {
...
function foreverStart(content, next) {
  ...
  _fe_startDaemon(path.resolve(process.cwd(), cont.script), cont.options,
    function(monitor, detach) {
      console.log('will hook', detach);
      monitor
        .on('start', function(process) {
          common.out.info(util.format('app started: pid %d.', process.childData.pid));
          detach(monitor);
          next(null);
        })
        .on('exit', function() {
